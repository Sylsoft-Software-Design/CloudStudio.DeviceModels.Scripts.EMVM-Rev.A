...
function getEndpoints(deviceAddress, endpoints)
{
  ...
  // temperature sensor, while the second one is a carbon dioxide sensor.
   endpoints.addEndpoint("1", "Sensor de Temperatura", endpointType.temperatureSensor);
   endpoints.addEndpoint("2", "Sensor de Humedad", endpointType.humiditySensor);
   
   var ep = endpoints.addEndpoint("3", "Sensor Genérico ", endpointType.genericSensor);
   ep.variableTypeId = 1141;

   var ep = endpoints.addEndpoint("4", "Estado del interruptor ", endpointType.genericSensor);
   ep.variableTypeId = 1142;

   var ep = endpoints.addEndpoint("6", "Conexion", endpointType.genericSensor);
   ep.variableTypeId = 1143;

   endpoints.addEndpoint("5", "Uptime ", endpointType.runTimeMeter)
}
